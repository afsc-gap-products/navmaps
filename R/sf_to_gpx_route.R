...
sf_to_gpx_route  <- function(x, file, name_col, time_col = NULL, color_col = NULL, description_col, ...) {
    ...
    for(ii in 1:nrow(x)) {
      ...
      for(jj in 1:nrow(unique_objects)) {
        ...
        lines <- c(lines, 
                   ...
                   "  </extensions>",
                   "  <rteseg>",
                   paste(paste0("    <rtept lat=\"", coords_sel[['Y']], "\" lon=\"", coords_sel[['X']],"\"></rtept>"), collapse = "\n"), 
                   "  </rteseg>",
                   "</rte>")
      }
    }
