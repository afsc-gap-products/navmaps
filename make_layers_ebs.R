...
for(ii in 1:length(software_types)) {
  ...
  # 14. Slope special project - marks
  slope_start <- readxl::read_xlsx(here::here("data", "special_projects", "slope_stations.xlsx")) |>
    sf::st_as_sf(coords = c("start_longitude", "start_latitude"), crs = "EPSG:4326") |>
    dplyr::mutate(depth = bottom_depth,
                  temperature = gear_temperature)
  slope_start$shape <- navmaps_sym_pal(values = "diamond", 
                                       software_format = SOFTWARE, 
                                       file_type = FILE_TYPE_POINT)
  slope_start$color <- navmaps_pal(values = "lightgreen", 
                                   software_format = SOFTWARE, 
                                   file_type = FILE_TYPE_POINT)
  slope_start$description <- slope_start$stationid
  
  
  sf_to_nav_file(x = slope_start,
                 file = here::here("output", region,  "navigation", SOFTWARE, paste0("slope_start_special_project.", FILE_TYPE_POINT)),
                 name_col = "description",
                 description_col = "description",
                 color_col = "color",
                 shape_col = "shape",
                 extra_cols = c("depth", "temperature"),
                 software_format = SOFTWARE)
  
  
  slope_path <- readxl::read_xlsx(here::here("data", "special_projects", "slope_stations.xlsx")) |>
    sf::st_as_sf(coords = c("start_longitude", "start_latitude"), crs = "EPSG:4326") |>
    dplyr::select(-end_latitude, -end_longitude) |>
    dplyr::bind_rows(  readxl::read_xlsx(here::here("data", "special_projects", "slope_stations.xlsx")) |>
                         sf::st_as_sf(coords = c("end_longitude", "end_latitude"), crs = "EPSG:4326") |>
                         dplyr::select(-start_latitude, -start_longitude)) |>
    dplyr::select(-strata, -wire, -dist_fished, -acc, -net_width, -surface_temperature) |>
    dplyr::group_by(stationid) %>% 
    dplyr::summarize(bottom_depth = mean(bottom_depth),
                     gear_temperature = mean(gear_temperature)) %>% 
    sf::st_cast("LINESTRING")
  
  slope_path$color <- navmaps_pal(values = "lightgreen", 
                                   software_format = SOFTWARE, 
                                   file_type = FILE_TYPE_LINESTRING)
  slope_path$description <- slope_path$stationid
 
  sf_to_nav_file(x = slope_path,
                 file = here::here("output", region,  "navigation", SOFTWARE, paste0("slope_path_special_project.", FILE_TYPE_LINESTRING)),
                 name_col = "description",
                 description_col = "description",
                 color_col = "color",
                 software_format = SOFTWARE)
