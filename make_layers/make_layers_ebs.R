...
for(ii in 1:length(software_types)) {
  ...
  bonus_15_30 <- sf::st_read(here::here("assets", "data", "special_projects", "EBS", "2024", "1530_bonus_zones.shp"))
  
  bonus_15_30$color <- navmaps_pal(values = "cyan",
                                   software_format = SOFTWARE,
                                   file_type = FILE_TYPE_POLYGON)
  bonus_15_30$fill <- 0
  bonus_15_30$name <- "15_30"
  
  sf_to_nav_file(x = bonus_15_30,
                 file = here::here("output", region,  "navigation", SOFTWARE, paste0("15_30_bonus.", FILE_TYPE_POLYGON)),
                 name_col = "name",
                 description_col = "CLUSTER",
                 color_col = "color",
                 shape_col = "shape",
                 fill_col = "fill",
                 software_format = SOFTWARE)
